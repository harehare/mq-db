...
`mq-db find` is a shortcut for `match()`/`bm25()`, no SQL needed. It falls back to a case-insensitive substring match too, so partial CJK queries still hit (see [`tokenize`](https://github.com/harehare/mq-db/blob/main/src/indexes.rs)'s known limitations). Results show a snippet centred on the match, with matched terms highlighted when stdout is a terminal (respects `NO_COLOR`):
...
```text
docs/API.md  ¶   1.92  Error handling follows RFC 7807 problem details...
docs/API.md  #   0.71  Error Handling
